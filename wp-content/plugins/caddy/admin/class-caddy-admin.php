...
class Caddy_Admin {
		...
		if ( $add_on_html_flag ) {
			...
			$caddy_addons_array = [
				...
				'ga-event'           => [
					...
					'btn_title'   => __( 'Get Add-on', 'caddy' ),
					'btn_link'    => 'https://www.usecaddy.com/?utm_source=caddy-addons&utm_medium=plugin&utm_campaign=addon-links',
					'activated'   => in_array( 'caddy-ga/ee-ga-events.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ? 'true' : 'false',
					'license'     => ( 'valid' === $caddy_ga_license_status ) ? 'activated' : 'not_activated',
				],
			];
	...
	public function caddy_header_links_html() {
		...
		<div class="cc-header-links">
			...
			| <a href="https://wordpress.org/support/plugin/caddy/reviews/#new-post" target="_blank"><?php echo esc_html( __( 'Show Some ♥', 'caddy' ) ); ?></a>
			| <a href="?page=caddy-addons"><?php echo esc_html( __( 'Add-ons', 'caddy' ) ); ?></a>
			<?php
			$caddy_license_status = get_option( 'caddy_premium_edd_license_status' );
			...
			if ( ! isset( $caddy_license_status ) || 'valid' !== $caddy_license_status ) {
				?>
				| <a href="https://www.usecaddy.com" target="_blank"><?php echo esc_html( __( 'Go Premium', 'caddy' ) ); ?></a>
				<?php
			} ?>
		</div>
		<?php
	...
	public function caddy_submit_deactivation_form_data() {
		...
		global $wp_version;
		$current_user          = wp_get_current_user();
		$popup_selected_reason = filter_input( INPUT_POST, 'popUpSelectedReason', FILTER_SANITIZE_STRING );
		$deactivation_reason   = filter_input( INPUT_POST, 'deactivationReason', FILTER_SANITIZE_STRING );
		$contact_me_checkbox   = filter_input( INPUT_POST, 'contactMeCheckbox', FILTER_SANITIZE_STRING );
